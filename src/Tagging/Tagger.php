...
class Tagger
{
    ...
    public function tagMp3File($file, $tagData, $coverFile = null)
    {
        if (!file_exists($file)) {
    ...
    public function tagFlacFile($file, $tagData, $allowBlank = false, $coverFile = null)
    {
        if (!file_exists($file)) {
            $error = 'The input file appears not to exist';
            return array('result' => false, 'error' => $error);
    ...
    public function embedFlacArt($imageFile, $audioFile)
    {
        // If setlocale(LC_CTYPE, "en_US.UTF-8") is not called here, any UTF-8 character will equate to an empty string.
    ...
    public function removeArtwork($file)
    {
        // If setlocale(LC_CTYPE, "en_US.UTF-8") is not called here, any UTF-8 character will equate to an empty string.
